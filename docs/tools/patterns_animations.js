(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () { document.querySelectorAll('.card').forEach(card => { card.classList.add('reveal'); card.addEventListener('click', () => { const href = card.getAttribute('data-href'); const title = (card.querySelector('h3') && card.querySelector('h3').innerText) || ''; const htmlPage = 'All_Design_Patterns_' + slugifyTitle(title).replace(/-/g, '') + '.html'; window.location.href = htmlPage; }); card.addEventListener('keypress', (e) => { if (e.key === 'Enter') { card.click(); } }); }); });
})();
